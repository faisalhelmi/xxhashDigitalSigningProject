...
class signScreen(QDialog):
    ...
    def runSign(self):
        ...
        hashed = int(xxhash.xxh3_128(hashing.encode()).hexdigest(), 16)
        ...
        signing_time = str(round(s1-s, 10))
        msg = "Signing Time : " + signing_time + " seconds"
        self.info_msg("Signing Success!\n" + "Lihat Signed File pada Folder File!\n", msg)
...
class verifyScreen(QDialog):
    ...
    def runVerify(self):
        ...
        hashed = int(xxhash.xxh3_128(self.message.encode()).hexdigest(), 16)
        
        if (self.nKey.text() != "" and self.eKey.text() != ""):
            if self.signature != "":
                verify = self.RSA.rsa_verify(self.signature, self.key[1], self.key[0], hashed)
                s1 = time.time()
                verify_time = str(round(s1-s, 10))
                if verify:
                    msg = "Verifying Time : " + verify_time + " seconds"
                    self.info_msg("Verified!\n", msg)
                else:
                    msg = ""
                    self.info_msg("Unverified!", msg)
            else:
                msg = ""
                self.info_msg("Signature not found!", msg)
        else:
            msg = ""
            self.info_msg("Key not found!", msg)
